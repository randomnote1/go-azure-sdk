...
func TestNewGroupIdSiteIdListIdItemID(t *testing.T) {
	id := NewGroupIdSiteIdListIdItemID("groupId", "siteId", "listId", "listItemId")
...
func TestFormatGroupIdSiteIdListIdItemID(t *testing.T) {
	actual := NewGroupIdSiteIdListIdItemID("groupId", "siteId", "listId", "listItemId").ID()
	expected := "/groups/groupId/sites/siteId/lists/listId/items/listItemId"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseGroupIdSiteIdListIdItemID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupIdSiteIdListIdItemId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/groups/groupId/sites/siteId/lists/listId/items/listItemId/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseGroupIdSiteIdListIdItemID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupIdSiteIdListIdItemIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupIdSiteIdListIdItemId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdSiteIdListIdItemIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupIdSiteIdListIdItemId(t *testing.T) {
	segments := GroupIdSiteIdListIdItemId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupIdSiteIdListIdItemId has no segments")
	}
