...
func TestParseGroupIdSiteIdContentTypeID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdSiteIdContentTypeID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupIdSiteIdContentTypeIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupIdSiteIdContentTypeId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdSiteIdContentTypeIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupIdSiteIdContentTypeId(t *testing.T) {
	segments := GroupIdSiteIdContentTypeId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupIdSiteIdContentTypeId has no segments")
	}
