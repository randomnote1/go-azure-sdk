...
func TestParseUserIdChatID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdChatID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserIdChatIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdChatId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdChatIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserIdChatId(t *testing.T) {
	segments := UserIdChatId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserIdChatId has no segments")
	}
