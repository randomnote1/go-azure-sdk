...
func TestNewGroupIdConversationIdThreadIdPostIdMentionID(t *testing.T) {
	id := NewGroupIdConversationIdThreadIdPostIdMentionID("groupId", "conversationId", "conversationThreadId", "postId", "mentionId")
...
func TestFormatGroupIdConversationIdThreadIdPostIdMentionID(t *testing.T) {
	actual := NewGroupIdConversationIdThreadIdPostIdMentionID("groupId", "conversationId", "conversationThreadId", "postId", "mentionId").ID()
	expected := "/groups/groupId/conversations/conversationId/threads/conversationThreadId/posts/postId/mentions/mentionId"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseGroupIdConversationIdThreadIdPostIdMentionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupIdConversationIdThreadIdPostIdMentionId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		{
			// Incomplete URI
			Input: "/groups/groupId/conversations/conversationId/threads/conversationThreadId/posts/postId/mentions",
			Error: true,
		},
		{
			// Valid URI
			Input: "/groups/groupId/conversations/conversationId/threads/conversationThreadId/posts/postId/mentions/mentionId",
			Expected: &GroupIdConversationIdThreadIdPostIdMentionId{
				GroupId:              "groupId",
		...
		},
		{
			...
			Input: "/groups/groupId/conversations/conversationId/threads/conversationThreadId/posts/postId/mentions/mentionId/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseGroupIdConversationIdThreadIdPostIdMentionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupIdConversationIdThreadIdPostIdMentionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupIdConversationIdThreadIdPostIdMentionId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdConversationIdThreadIdPostIdMentionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupIdConversationIdThreadIdPostIdMentionId(t *testing.T) {
	segments := GroupIdConversationIdThreadIdPostIdMentionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupIdConversationIdThreadIdPostIdMentionId has no segments")
	}
