...
func TestParseMeTransitiveMemberOfID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseMeTransitiveMemberOfID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeTransitiveMemberOfIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeTransitiveMemberOfId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeTransitiveMemberOfIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeTransitiveMemberOfId(t *testing.T) {
	segments := MeTransitiveMemberOfId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeTransitiveMemberOfId has no segments")
	}
