...
func TestParseGroupIdTeamOwnerID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdTeamOwnerID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupIdTeamOwnerIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupIdTeamOwnerId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdTeamOwnerIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupIdTeamOwnerId(t *testing.T) {
	segments := GroupIdTeamOwnerId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupIdTeamOwnerId has no segments")
	}
