...
func TestParseUserIdEventID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdEventID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserIdEventIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdEventId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdEventIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserIdEventId(t *testing.T) {
	segments := UserIdEventId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserIdEventId has no segments")
	}
