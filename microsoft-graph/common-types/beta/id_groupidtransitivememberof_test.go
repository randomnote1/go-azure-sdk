...
func TestParseGroupIdTransitiveMemberOfID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdTransitiveMemberOfID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseGroupIdTransitiveMemberOfIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *GroupIdTransitiveMemberOfId
	...
	for _, v := range testData {
		...
		actual, err := ParseGroupIdTransitiveMemberOfIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForGroupIdTransitiveMemberOfId(t *testing.T) {
	segments := GroupIdTransitiveMemberOfId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("GroupIdTransitiveMemberOfId has no segments")
	}
