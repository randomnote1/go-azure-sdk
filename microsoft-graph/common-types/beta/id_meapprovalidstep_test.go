...
func TestParseMeApprovalIdStepID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseMeApprovalIdStepID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeApprovalIdStepIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeApprovalIdStepId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeApprovalIdStepIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeApprovalIdStepId(t *testing.T) {
	segments := MeApprovalIdStepId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeApprovalIdStepId has no segments")
	}
