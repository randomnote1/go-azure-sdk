...
func TestParseUserIdExtensionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdExtensionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserIdExtensionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdExtensionId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdExtensionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserIdExtensionId(t *testing.T) {
	segments := UserIdExtensionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserIdExtensionId has no segments")
	}
