...
func TestNewUserIdJoinedTeamIdChannelIdMessageIdReplyID(t *testing.T) {
	id := NewUserIdJoinedTeamIdChannelIdMessageIdReplyID("userId", "teamId", "channelId", "chatMessageId", "chatMessageId1")
...
func TestFormatUserIdJoinedTeamIdChannelIdMessageIdReplyID(t *testing.T) {
	actual := NewUserIdJoinedTeamIdChannelIdMessageIdReplyID("userId", "teamId", "channelId", "chatMessageId", "chatMessageId1").ID()
	expected := "/users/userId/joinedTeams/teamId/channels/channelId/messages/chatMessageId/replies/chatMessageId1"
	if actual != expected {
		t.Fatalf("Expected the Formatted ID to be %q but got %q", expected, actual)
	}
}

func TestParseUserIdJoinedTeamIdChannelIdMessageIdReplyID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdJoinedTeamIdChannelIdMessageIdReplyId
	}{
		{
			// Incomplete URI
			Input: "",
		...
		},
		{
			...
			Input: "/users/userId/joinedTeams/teamId/channels/channelId/messages/chatMessageId/replies/chatMessageId1/extra",
			Error: true,
		},
	}
	for _, v := range testData {
		...
		actual, err := ParseUserIdJoinedTeamIdChannelIdMessageIdReplyID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserIdJoinedTeamIdChannelIdMessageIdReplyIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdJoinedTeamIdChannelIdMessageIdReplyId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdJoinedTeamIdChannelIdMessageIdReplyIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserIdJoinedTeamIdChannelIdMessageIdReplyId(t *testing.T) {
	segments := UserIdJoinedTeamIdChannelIdMessageIdReplyId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserIdJoinedTeamIdChannelIdMessageIdReplyId has no segments")
	}
