...
func TestParseUserIdRegisteredDeviceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdRegisteredDeviceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserIdRegisteredDeviceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdRegisteredDeviceId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdRegisteredDeviceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserIdRegisteredDeviceId(t *testing.T) {
	segments := UserIdRegisteredDeviceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserIdRegisteredDeviceId has no segments")
	}
