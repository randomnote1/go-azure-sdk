...
func TestParseIdentityGovernanceEntitlementManagementResourceRequestID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseIdentityGovernanceEntitlementManagementResourceRequestID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseIdentityGovernanceEntitlementManagementResourceRequestIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *IdentityGovernanceEntitlementManagementResourceRequestId
	...
	for _, v := range testData {
		...
		actual, err := ParseIdentityGovernanceEntitlementManagementResourceRequestIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForIdentityGovernanceEntitlementManagementResourceRequestId(t *testing.T) {
	segments := IdentityGovernanceEntitlementManagementResourceRequestId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("IdentityGovernanceEntitlementManagementResourceRequestId has no segments")
	}
