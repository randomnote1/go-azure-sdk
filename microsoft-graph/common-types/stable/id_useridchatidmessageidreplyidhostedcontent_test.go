...
func TestParseUserIdChatIdMessageIdReplyIdHostedContentID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdChatIdMessageIdReplyIdHostedContentID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserIdChatIdMessageIdReplyIdHostedContentIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdChatIdMessageIdReplyIdHostedContentId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdChatIdMessageIdReplyIdHostedContentIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserIdChatIdMessageIdReplyIdHostedContentId(t *testing.T) {
	segments := UserIdChatIdMessageIdReplyIdHostedContentId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserIdChatIdMessageIdReplyIdHostedContentId has no segments")
	}
