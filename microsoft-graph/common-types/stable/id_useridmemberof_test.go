...
func TestParseUserIdMemberOfID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdMemberOfID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserIdMemberOfIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdMemberOfId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdMemberOfIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserIdMemberOfId(t *testing.T) {
	segments := UserIdMemberOfId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserIdMemberOfId has no segments")
	}
