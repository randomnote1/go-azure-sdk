...
func TestParseUserIdOwnedDeviceID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdOwnedDeviceID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseUserIdOwnedDeviceIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *UserIdOwnedDeviceId
	...
	for _, v := range testData {
		...
		actual, err := ParseUserIdOwnedDeviceIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForUserIdOwnedDeviceId(t *testing.T) {
	segments := UserIdOwnedDeviceId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("UserIdOwnedDeviceId has no segments")
	}
