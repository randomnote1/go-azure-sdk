...
func TestParseMeOnenoteSectionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
	...
	for _, v := range testData {
		...
		actual, err := ParseMeOnenoteSectionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestParseMeOnenoteSectionIDInsensitively(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *MeOnenoteSectionId
	...
	for _, v := range testData {
		...
		actual, err := ParseMeOnenoteSectionIDInsensitively(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
...
func TestSegmentsForMeOnenoteSectionId(t *testing.T) {
	segments := MeOnenoteSectionId{}.Segments()
	if len(segments) == 0 {
		t.Fatalf("MeOnenoteSectionId has no segments")
	}
